...
#define MAX_REQ_PER_CLIENT 10
#define MAX_BUF_SIZE 138 

struct circular_queue
{
	int front, rear; 
	char req[MAX_REQ_PER_CLIENT][MESSAGE_SIZE];

};

	void enqueue(struct circular_queue *ptr, char *msg)
	{
		if(ptr->front == 0 && (ptr->rear == MAX_REQ_PER_CLIENT-1))
		{
	...
	void dequeue(struct circular_queue *ptr, char *msg)
	{
		if( ptr->front ==-1)
		{
		...
		strcpy( msg, ptr->req[ptr->front]);
		if( ptr->front== ptr->rear)
			ptr->front =ptr->rear =-1;
		else if( ptr->front == MAX_REQ_PER_CLIENT)
			ptr->front = 0;
		else 
			ptr->front++;
		return;
...
struct Client
{
	...
	struct circular_queue *pend_req;
    long int id;
};
...
int get_client_id( char *msg, char *msg_type)
{
	int i = 0, temp_id = 0; 
	while( msg[i] !=' ')
	{
...
void open_client_queue( char *queue_name, int place)
{
	mqd_t cqt;
	while(1)
	{
	...
	client_data[place].cqt = cqt;


}
void send_message( char *msg, char *str , int place)
{
	while(1)
	{


        if( mq_send( client_data[place].cqt, msg, strlen( msg)+1, 0) == -1)
        {
...
void initialise_client( long int client_id, long long file_size, struct Segment *ptr, int num_segments, char *queue_name, long seg_size)
{
    ...
    send_message(buffer2, "segment name sent to the client", i);
...
void process_message( char *msg, int size, char *queue_name, struct Segment *ptr, int num_segments, long segment_size)
{
	...
	//printf("entered the send type\n");
	    ...
	    client_data[place].done =1;
            send_message("ACK ", " take the data", place);
            send_message("REC ", " take the data", place);
	...
	{
            send_message("DONE ", "done compression",place);
	    client_data[place].new_req =1;
...
for( i =0; i<10; i++)
{
	...
	int cid = get_client_id( buffer1, msg_type); 
        int place = client_present( cid); 
	if(( place == -1) || ( strcmp(msg_type, "INIT") != 0))
	{
 
		 
 	       process_message( buffer1, strlen( buffer1),queue_name, segment_info, num_segments, segment_size );
	       if( client_data[place].new_req ==1) 
		{
			if( client_data[place].pend_req != NULL)
			{
				dequeue(client_data[place].pend_req, buffer1);
    				client_data[place].done =0;
				client_data[place].new_req =0;
    				client_data[place].offset = 0;
    				client_data[place].compressed_length = 0;

 	       			process_message( buffer1, strlen( buffer1),queue_name, segment_info, num_segments, segment_size );
			}
			else 
	    		cleanup_client( cid, segment_size, segment_info);
		}
        }
	...
	else 
	{
		if( client_data[place].pend_req == NULL)
		{
			client_data[place].pend_req = (struct circular_queue*) malloc( sizeof(struct circular_queue));
			client_data[place].pend_req->front =client_data[place].pend_req->rear = -1;
		}
		
			enqueue(client_data[place].pend_req, buffer1);
	}
